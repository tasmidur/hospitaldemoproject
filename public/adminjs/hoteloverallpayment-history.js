...
$(document).ready(function(){
  ...
  $(document).on("click","#witdrawbtn",function(event){
    let dateto=$("#to").val();
    let datefrom=$("#from").val();
    if(datefrom!='' && dateto!=''){
    $.ajax({
      url:'/hotelbooking/hotel-payment-witdraw',
      type:"post",
      data:{
        from_date:datefrom,
        to_date:dateto
      ...
      success:function(res){
        if(res){
            showNotification("Successfully added to the withdraw history",1);
        }
      }
    });
    }else{
        showNotification("Please,Select a date range for withdraw.",1);
    }
  });
...
function data_load(from_date='',to_date=''){
  ...
  serverSide: true,
  ajax: {
   url:'/hotelbooking/hotel-overall-payment-history',
